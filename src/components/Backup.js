...
const NotificationsModal = ({ isVisible = true }) => {
...
const items = [
    { action: "edit", item: "Edit Profile" },
    { action: "delete", item: "Delete Account" },
    { action: "settings", item: "Account Settings" },
    { action: "logout", item: "Log Out" },
  ];
      ...
      <div className="bg-white border border-gray-300 rounded-md p-4 shadow-lg">
        <ul className="space-y-0">
          {items.map((item, idx) => (
            <li 
              key={idx} 
              className={'text-sm text-gray-800 pb-2 hover:cursor-pointer'}
